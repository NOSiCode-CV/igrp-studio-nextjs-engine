...
import { WorkspaceConfig, ProjectWorkspace, ServiceWorkspace } from '../src/interfaces/types';
...
describe('Workspace Migration - Package Independence Tests', () => {
  ...
  test('Should add project to workspace independently', async () => {
    ...
    const workspaceDir = path.join(OUTPUT_DIR, baseWorkspaceConfig.slug);
    const projectDir = path.join(workspaceDir, 'projects', testProjectConfig.basePath);
  ...
  test('Should update project in workspace', async () => {
    ...
    updatedProjectConfig.environments.push({ key: 'NEW_VAR', value: 'new_value' });
    updatedProjectConfig.ports.internal = 3001;
    ...
    expect(workspaceData.projects[0].environments).toHaveLength(2);
  });

  test('Should remove project from workspace', async () => {
    await removeProjectFromWorkspace(testProjectConfig.basePath, OUTPUT_DIR);
  ...
  test('Should remove service from workspace', async () => {
    await removeServiceToWorkspace(testServiceConfig.service.name, OUTPUT_DIR);
...
describe('Workspace Migration - Type System Independence Tests', () => {
  ...
  test('Should handle project workspace types independently', async () => {
    ...
    expect(testProject.basePath).toBe('type-test-project');
  });
