...
export const createAppDirectories = async (context: RenderContext) => {
  ...
  createStaticDir(context.basePath);
};
...
const createStaticDir = (basePath: string) => {
  fs.mkdirSync(path.join(basePath, DIRECTORIES.FIELDS), { recursive: true });
  fs.mkdirSync(path.join(basePath, DIRECTORIES.SERVICES), { recursive: true });
  fs.mkdirSync(path.join(basePath, DIRECTORIES.COMPONENTS), { recursive: true });
};