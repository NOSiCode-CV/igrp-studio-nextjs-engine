...
export const renderTableRow = function (config: Layout, registry: Record<string, Component>): string {
  ...
  return component.onTableComponent ?? (table.childrenTypes.has(config.componentName) ? renderLayout(config) : '<span className="font-medium">Unsupported Component</span>');
