...
import { normalizeHostname, normalizeDatabase, normalizeDatabaseImg, normalizeDefaultPort } from '../../utils/workspace-helper';
...
export const mapProjectToWorkspace = async (
  ...
  const workspace: WorkspaceProjectsConfig = await loadWorkspaceConfig(basePath);

  const addProjectId: string = config.config.id;
  const projectIndex: number = workspace.projects.findIndex((p: any) => p.config?.id === addProjectId);

  if (projectIndex === 1) {
    throw new Error(`Project with id "${addProjectId}" is in the workspace already`);
  }

  const isSpringBoot: boolean = config.config.type === 'springboot';
  const basePort: number = isSpringBoot ? 8083 : 3001;

  const index: number = isSpringBoot
    ? workspace.projects.filter((proj: any) => proj.config.type === 'springboot').length
    : workspace.projects.filter((proj: any) => !(proj.config.type === 'springboot')).length;

  const indexService: number = workspace.services.filter((serv: any) => serv.name === normalizeDatabase(config.config.database)).length
...
export const removeServiceInWorkspace = async (serviceId: string, basePath: string, ws?: WorkspaceProjectsConfig): Promise<WorkspaceProjectsConfig> => {
  ...
  const serviceIndex = workspace.services.findIndex((s: any) => s.id === serviceId);
